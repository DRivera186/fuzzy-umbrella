console.log('sessions controller');
var mongoose = require('mongoose')
var User = mongoose.model('User');
module.exports = {

    register: function(req,res){
        console.log('<--------------Made it to Register-------------------->'.red);
        if (req.body.password != req.body.pw_confirm){
            var noMatch = {errors: {password: {message:"Passwords did not match, Please try again."}}};
            res.json(noMatch);
        } else {
            var user = new User(req.body);
            user.save(function(err,user){
                if (err){
                    res.json(err);
                }else{
                    req.session.user = {
                        first_name: user.first_name,
                        _id: user._id
                    };
                    req.session.userId = user._id;
                    res.sendStatus(200);
                }
            });
        }
    },

    login: function(req,res) {
        console.log('<--------------Made it to login-------------------->'.red);
        console.log(req.body);
            User.findOne({email: req.body.email}).exec(function (err, user) {
                if(user) {
                    if (req.body.password == user.password) {
                        console.log('Password is a go!'.blue);
                        req.session.user = {
                            first_name: user.first_name,
                            _id: user._id
                        };
                        req.session.userId = user._id;
                        res.json(user)
                    } else {
                        console.log('User does not exist, Please try again.');
                        var wrongUser = {errors: {password: {message:"Password does not match."}}};
                        res.json(wrongUser);
                    }
                } else {
                    var noUser = {errors: {password: {message:"User does not exist, Please try again."}}};
                    res.json(noUser);
                }
            })
    },

    logout: function(req, res) {
        console.log('<--------------Made it to logout Method-------------------->'.red);
        req.session.userId = null;
        console.log('logged out');
        console.log(req.session.userId);
        res.sendStatus(401);
    }
}











